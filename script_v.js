/* ==========================================================
   script_v.js — final synchronized
   - Auth + theme + navigation
   - Image upload + coco-ssd scan-only
   - Enhancer: OCR/HDR, inline preview
   - Annotation: rect, arrow, text, highlight, blur, freehand
   - Tooltips on every help-tip and data-tip attributes
   - Uses the uploaded preview image at /mnt/data/ee5ca6b2...png
   =========================================================*/
...
function applyThemeValue(key){
  ...
  if(themeToggle) themeToggle.textContent = (document.documentElement.classList.contains("theme-light") ? "☀️" : "🌙");
}
...
/* tooltips: any element with data-tip or .help-tip shows tooltip */
...
function attachTooltips(){
  document.querySelectorAll("[data-tip], .help-tip").forEach(el=>{
    // remove existing to avoid double attach
    el.onmouseenter = el.onmouseleave = null;
    el.addEventListener("mouseenter", ()=>{
      const tip = el.dataset.tip || el.getAttribute("data-tip") || el.title || el.getAttribute("title") || "Info";
...
/* ==========================================================
   IMAGE RESIZER (scan-only, optional coco-ssd)
==========================================================*/
...
/* load image and set preview (if user uploads) */
...
on(enhPreviewBtn, "click", ()=>{
  if(!enhanceCanvas.width) {
    // if no uploaded image, still show sample image (we set sample in HTML)
    return alert("Upload an image first to preview.");
  }
...
/* run enhancement -> filters + merge -> download */
on(enhRunBtn, "click", ()=>{
...
/* merge annotations into image natural-size canvas */
function mergeAnnotationsToEnhanceCanvas(){
  ...
  for(const a of actionsStack){
    if(a.tool === "text"){
      ...
      tctx.font = `${Math.max(12, a.size*3) * scaleX}px Inter, system-ui, sans-serif`;
  ...
  for(let row=0; row<h; row++){
    for(let col=0; col<w; col++){
      ...
      for(let k=-half;k<=half;k++){
        ...
        const idx = (row*w + x)*4; const wt = weights[k+half];
  ...
  for(let col=0; col<w; col++){
    for(let row=0; row<h; row++){
      ...
      for(let k=-half;k<=half;k++){
        ...
        const idx = (y*w + col)*4; const wt = weights[k+half];
...
/* init handlers */
window.addEventListener("resize", ()=> { ensureAnnoCanvas(); attachTooltips(); });
ensureAnnoCanvas();
attachTooltips();
...
document.addEventListener("keydown", (e)=>{
  if(e.key === "Escape"){
    ["aboutModal","themeModal"].forEach(id=>{ const m = $(id); if(m) m.style.display = "none"; });
  }
});
